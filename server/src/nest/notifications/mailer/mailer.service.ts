...
/** Transient network codes that are worth retrying once after a short delay. */
const RETRYABLE_CODES = new Set(['ETIMEDOUT', 'ECONNRESET', 'ECONNREFUSED', 'ENOTFOUND']);
const RETRY_DELAY_MS = 2_000;
const APP_NAME = 'TrekFlow';

/**
 * Retries `fn` once after RETRY_DELAY_MS when it throws a transient SMTP error.
 * Non-retryable errors (wrong password, bad envelope, etc.) are re-thrown immediately.
 */
async function withSmtpRetry<T>(fn: () => Promise<T>): Promise<T> {
  try {
    return await fn();
  } catch (err) {
...
export class MailerService {
  ...
  /**
   * Is SMTP configured at the instance level?
   * All three required fields (host, port, from) must be present.
   */
  ...
  async sendPasswordResetEmail(
    ...
    try {
      await withSmtpRetry(() =>
        this.createTransport(smtpCfg).sendMail({
          from: smtpCfg.from,
          to,
  ...
  async sendEmail(
    ...
    try {
      await withSmtpRetry(() =>
        this.createTransport(config).sendMail({
          from: config.from,
          to,
