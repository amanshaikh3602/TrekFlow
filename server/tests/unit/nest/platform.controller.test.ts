...
function makeRes() {
  const res = {
    ...
    redirect: vi.fn(function (this: typeof res, url: string) { this.body = `REDIRECT:${url}`; return this; }),
  };
  return res;
}
...
describe('SpaFallbackFilter', () => {
  ...
  it('keeps the JSON 404 envelope outside production even for GET on non-root paths', () => {
    process.env.NODE_ENV = 'development';
    const res = makeRes();
    new SpaFallbackFilter().catch(new NotFoundException('missing'), host({ method: 'GET', path: '/api/missing' }, res));
    expect(res.statusCode).toBe(404);
    expect(res.body).toEqual({ error: 'missing' });
  });

  it('redirects root GET to frontend dev server in development', () => {
    process.env.NODE_ENV = 'development';
    const res = makeRes();
    new SpaFallbackFilter().catch(new NotFoundException(), host({ method: 'GET', path: '/' }, res));
  ...
  it('falls back to Not Found when the exception has no message', () => {
    ...
    new SpaFallbackFilter().catch(exc, host({ method: 'GET', path: '/api/missing' }, res));
